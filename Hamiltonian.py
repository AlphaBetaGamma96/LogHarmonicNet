...
class HarmonicOscillatorWithInteraction1D(nn.Module):
  ...
  def __init__(self, network, V0, sigma0):
    ...
    self.kinetic_const = -(self.hbar**2)/(2*self.mass) #MeV fm^2
    self.potential_const = 0.5*self.mass*self.omega**2 #MeV fm^-2
    self.gauss_const = self.V/(np.sqrt(2*np.pi)*self.sigma)
  ...
  def local_gaussian_interaction_per_walker(self, xs):
    """
    Method to calculate a short-ranged gaussian interaction between two fermions
    """
    return torch.stack([self.gauss_const*torch.exp( -(xs[:,i]-xs[:,j])**2/(2*self.sigma**2) ) for i in range(xs.shape[1]) for j in range(i+1, xs.shape[1])], dim=-1).sum(dim=-1)
  
  def local_gaussian_interaction(self, x):
    """
    Method to calculate the total gaussian inter-particle interaction between fermions
    """
    return torch.stack([self.local_gaussian_interaction_per_walker(x[walker, :, :,]) for walker in range(x.shape[0])], dim=0)
  
  def forward(self, x):
    ek_local = self.local_kinetic_from_log(x)
    ep_local = self.local_potential(x)
    ep_int_local = self.local_gaussian_interaction(x)
    e_local = ek_local + ep_local + ep_int_local
    return e_local   
