...
from Hamiltonian import HarmonicOscillator1D as HO1D
...
nfermions = 12   #number of input nodes
...
nblocks = 100             #number of blocks in Markov Chain
nsteps = 100              #number of samples within a given block
nsamples = nblocks*nsteps #total number of samples within a Markov Chain
burn_in = 0               #number of burn in samples of Markov Chain
thinning = 20             #thinning factor for samples (take every thinning-th sample, i.e. if thinning = 10, take every 10-th sample from chain)
nwalkers = 100            #number of chains/walkers 
std = 0.5                 #width of the proposal distribution
...
calc_local_energy = HO1D(network=net) #class that calculates the local matrix elements of the Hamiltonian
...
fname = "A%02i_H%03i_B%03i_S%03i.csv" % (nfermions, num_hidden, nblocks, nsteps)
